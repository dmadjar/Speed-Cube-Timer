...
struct threeStats: View {
    @Environment(\.managedObjectContext) var managedObjContext
    @FetchRequest(sortDescriptors: [SortDescriptor(\.date, order: .reverse)]) var threeSolves: FetchedResults<ThreeSolve>
    
    var sumTime : Double {
        threeSolves.reduce(0) { $0 + $1.timeDouble }
    }
    
    var runningAvg : Double {
        sumTime / Double(threeSolves.count)
    }
   ...
   // @EnvironmentObject var trash: trashEnabled
                ...
                ZStack {
                    ...
                    VStack {
                        Spacer()
                            ...
                            Text("Solve Count: \(threeSolves.count)")
                            ...
                            Text(threeSolves.count == 0 ? "Running Average: No Solves" : "Running Average: \(timerManager.getFormatedTime(seconds: runningAvg))")
                                    .font(.custom("Roboto-Black", size: UIScreen.main.bounds.width / 22))
                                    .foregroundColor(Color("textColor"))
                                    .frame(width: UIScreen.main.bounds.width / 1.2, alignment: .leading)
                        ...
                        ScrollView {
                            ForEach(threeSolves) { solve in
                                NavigationLink(destination: scrambleDisplay(scramble: solve.scramble!, time: solve.timeString!, color: Color("blueColor"))) {
                                    VStack {
                                        Spacer()
                                            .frame(height: 15)
                                        ...
                                        HStack {
                                          ...
                                          if trashEnabled.isDeleting {
                                                    Button(action: {
                                                        if let index = threeSolves.firstIndex(of: solve) {
                                        ...
                                        RoundedRectangle(cornerRadius: 15)
                                            .fill(Color("darkBlue").opacity(0.10))
                                            .frame(width: UIScreen.main.bounds.width / 1.25, height: 3)
                                    }
                                            Spacer()
                                                .frame(height: 15)
    ...
    func lowestTime() -> Double {
        var lowest = threeSolves.first?.timeDouble
        for solve in threeSolves {
...
struct threeStats_Previews: PreviewProvider {
    
    static let trashEnabled = TrashEnabled()
  //  static var trash =  trashEnabled()
...
// scrambleDisplay(scramble: solve.scramble!, time: solve.timeString!, color: Color("blueColor"))
