...
struct twoStats: View {
    @Environment(\.managedObjectContext) var managedObjContext
    @FetchRequest(sortDescriptors: [SortDescriptor(\.date, order: .reverse)]) var twoSolves: FetchedResults<TwoSolve>
    
    
    var sumTime : Double {
        twoSolves.reduce(0) { $0 + $1.timeDouble }
    }
    
    var runningAvg : Double {
        sumTime / Double(twoSolves.count)
    }
            ...
            ZStack {
                ...
                VStack {
                    Spacer()
                        ...
                        Text("Solve Count: \(twoSolves.count)")
                        ...
                        Text(twoSolves.count == 0 ? "Running Average: No Solves" : "Running Average: \(timerManager.getFormatedTime(seconds: runningAvg))")
                                .foregroundColor(Color("textColor"))
                                .font(.custom("Roboto-Black", size: UIScreen.main.bounds.width / 22))
                                .frame(width: UIScreen.main.bounds.width / 1.2, alignment: .leading)
                    ...
                    ZStack {
                        ...
                        VStack(spacing: 3) {
                            ...
                            RoundedRectangle(cornerRadius: 30)
                                .fill(Color("darkYellow").opacity(0.25))
                                .frame(width: UIScreen.main.bounds.width / 1.25, height: 3)

                    ScrollView {
                        ForEach(twoSolves) { solve in
                            NavigationLink(destination: scrambleDisplay(scramble: solve.scramble!, time: solve.timeString!, color: Color("yellowColor"))) {
                                VStack {
                                    Spacer()
                                        .frame(height: 15)
                                    ...
                                    HStack {
                                        ...
                                        if trashEnabled.isDeleting {
                                                Button(action: {
                                                    if let index = twoSolves.firstIndex(of: solve) {
                                    ...
                                    RoundedRectangle(cornerRadius: 15)
                                        .fill(Color("darkYellow").opacity(0.10))
                                        .frame(width: UIScreen.main.bounds.width / 1.25, height: 3)
                                    }
                                        Spacer()
                                            .frame(height: 15)
    ...
    func lowestTime() -> Double {
        var lowest = twoSolves.first?.timeDouble
        for solve in twoSolves {
...
struct twoStats_Previews: PreviewProvider {
